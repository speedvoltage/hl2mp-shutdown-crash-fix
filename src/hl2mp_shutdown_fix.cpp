...
#error HL2MP Shutdown Fix supports Linux only.
#endif

#if !defined(__x86_64__) || !defined(PLATFORM_64BITS) || !defined(X64BITS)
#error HL2MP Shutdown Fix supports x86-64 only.
#endif

#if !defined(HL2MP)
#error HL2MP Shutdown Fix must be built with HL2MP defined.
#endif
...
public:
    ...
    {
        if (g_RejectionReason[0])
        {
            CopyError(error, maxlength, g_RejectionReason);
            return false;
        }
        ...
        PrintMeta("Loaded HL2MP Shutdown Fix");
        return true;
    }
    ...
    {
        PrintMeta("HL2MP Shutdown Fix unloaded, but retaining shutdown fix!");
        return true;
    }
    ...
    const MetamodLoaderInfo *)
{
    ...
    if (version->source_engine != SOURCE_ENGINE_HL2DM)
        snprintf(g_RejectionReason, sizeof(g_RejectionReason), "%s", "HL2MP Shutdown Fix only supports Metamod's HL2DM engine provider");

    const char *gameDirectory = version->GetGameDir();
    if (!gameDirectory || strcmp(gameDirectory, "hl2mp") != 0)
        snprintf(g_RejectionReason, sizeof(g_RejectionReason), "%s", "HL2MP Shutdown Fix only supports the hl2mp game directory");
